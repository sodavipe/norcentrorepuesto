...
export class EditNewCuponeComponent implements OnInit {
  ...
  showCupon(){
    this._cuponService.showCupon(this.cupon_id).subscribe((resp:any)=>{
      console.log(resp)
      ...
      this.type_segment = this.cupon_selected.type_segment;
      this.state = this.cupon_selected.state ? this.cupon_selected.state : 1;
      if(this.type_segment == 1){
        this.cupon_selected.products.forEach(product_s => {
        ...
        this.cupon_selected.categories.forEach(product_s => {
